...
extension Weakifiable where Self: Sendable {
	/// Weakly captures an object in non-parametrized void result closure.
	///
	/// Creates `() -> Void` handler from `(Object) -> Void` closure
	/// so you can access weakly captured Object in your handler if the object is present.
	...
	public func _capture(
		in closure: @escaping @Sendable (Self) -> Void
	) -> @Sendable () -> Void {
		return Weak(self)._capture(in: closure)
	}
...
extension Weakifiable where Self: Sendable {
	...
	public func _capture<Output>(
		...
		in closure: @escaping @Sendable (Self) -> Output
	) -> @Sendable () -> Output {
		return Weak(self)._capture(orReturn: defaultValue(), in: closure)
	}
